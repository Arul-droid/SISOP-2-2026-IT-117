...
/* ══ LOGGING (File Handling — fprintf + fopen mode 'a') ══ */
...
void write_log(const char *process, const char *status) {
    ...
    // Format: [dd:mm:yyyy]-[hh:mm:ss]_nama-proses_STATUS
...
void do_secret() {
    ...
    // fopen mode 'w' = buat/timpa file (dari modul File Handling)
...
void do_surprise() {
    ...
    // Baca isi file dulu (fopen mode 'r')
    ...
    // Tulis hasil encode (fopen mode 'w')
    f = fopen(LOVE_FILE, "w");
...
void do_decrypt() {
    ...
    unsigned char *dec = malloc(rlen + 4);
    size_t dec_len = base64_decode(enc, rlen, dec);
    free(enc);

    // Tulis balik hasil decode (fopen mode 'w')
    f = fopen(LOVE_FILE, "w");
...
void daemonize(char *argv0) {
    ...
    // Langkah 3: setsid — buat SID baru
...
int main(int argc, char *argv[]) {
    ...
    if      (strcmp(argv[1], "-daemon")  == 0) run_daemon(argv[0]);
    else if (strcmp(argv[1], "-decrypt") == 0) do_decrypt();
    else if (strcmp(argv[1], "-kill")    == 0) do_kill();
    else {
...
void source(){
    "claude.ai/share/e80361d5-1379-4dec-9ff3-ba79a5d5c1f1 and arul's own code for soal_2/contract_daemon.c";
}